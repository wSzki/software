...
class FaderfoxHelper:
    ...
    def selected_scene_idx(self):
        ...
        def tuple_idx(tuple, obj):
            """
            Returns the index of the element of tuple that is == obj
            """
    ...
    def toggle_clip_playing(self, track_idx):
        """
        Toggles the playing clip in the currently selected scene. If it is playing, stop() it, else fire() it
        """
    ...
    def is_rack(self, device):
        """
        Returns true if the device is a rack
        """
        deviceName = self.device_name(device)
        return deviceName in ["DrumGroupDevice",
    ...
    def get_visible_track(self, idx):
        tracks = list(self.song().visible_tracks) + list(self.song().return_tracks) + [self.song().master_track]

        if idx < len(tracks):
            return tracks[idx]
        else:
            if self.is_master_track_selected():
                self.parent.log("master is selected, jumping to max")
                new_idx = min(15, len(tracks) - 1)
                return self.get_visible_track(new_idx)
            else:
                return self.song().master_track

    def get_track_idx(self, track):
        tracks = list(self.song().tracks) + list(self.song().return_tracks) + [self.song().master_track]
        idx = 0
    ...
    def select_visible_track_by_idx(self, idx):
        track = self.get_visible_track(idx)
        self.parent.log("selecting track for idx %s: %s" % (idx, track))
        self.song().view.selected_track = track

    def selected_visible_track_idx(self):
        def tuple_idx(_tuple, obj):
            for i in xrange(0, len(_tuple)):
                if not cmp(_tuple[i], obj):
                    return i
        return tuple_idx(list(self.song().visible_tracks) + list(self.song().return_tracks) + [self.song().master_track], \
    ...
    def selected_track_idx(self):
        def tuple_idx(_tuple, obj):
            for i in xrange(0, len(_tuple)):
                if not cmp(_tuple[i], obj):
                    return i
        return tuple_idx(list(self.song().tracks) + list(self.song().return_tracks) + [self.song().master_track], \
                         self.song().view.selected_track)
